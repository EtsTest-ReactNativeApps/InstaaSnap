			...
			.then(async (response) => {
				if (response.data.mediaList.length === 1) {
					...
					if (response.data.mediaList[0].search('.mp4') === -1) {
						saveAs(imageBlob, Math.floor(Math.random() * Math.pow(15, 15)) + '.jpg');
						setActive(false);
					} else {
						saveAs(imageBlob, Math.floor(Math.random() * Math.pow(15, 15)) + '.mp4');
						setActive(false);
					}
					...
					for (let mediaFile of response.data.mediaList) {
						...
						} else mediaName = Math.floor(Math.random() * Math.pow(15, 15)) + '.mp4';
					...
					folder.generateAsync({ type: 'blob' }).then((content) => {
						saveAs(content, Math.floor(Math.random() * Math.pow(15, 15)) + '.zip');
						setActive(false);
					});
				}
			})
